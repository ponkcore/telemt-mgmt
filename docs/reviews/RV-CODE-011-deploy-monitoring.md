...
### Low  (optional)
- **F-L1:** `infra/monitoring/docker-compose.yml:28` — Prometheus is configured with `--alertmanager.url=http://localhost:9093`, but no Alertmanager service is deployed (Alertmanager Telegram integration is deferred per TKT-011@0.1.1 §3). Either remove the flag or add a backlog ticket to deploy Alertmanager.
